...
function getSignupFormErrors(name, email, password, confirmPassword) {
    ...
    if (!confirm_password_input.parentElement.classList.contains('incorrect')) {
            confirm_password_input.parentElement.classList.add('incorrect')
        }
    }
    return errors;
...
function getLoginFormErrors(email, password){
    ...
    if(password === '' || password == null) {
        errors.push('Password is required')
        password.push.parentElement.classList.add('incorrect')
    }

    return errors;
}

const allInputs = [name_input, email_input, password_input, confirm_password_input].filter(input => input !=
    null)
